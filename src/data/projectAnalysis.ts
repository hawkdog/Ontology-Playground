...
export const sampleProjectAnalysis: ProjectAnalysisModel = {
  ...
  repositories: [
    ...
    },
    {
      ...
      type: 'service',
      visibility: 'public',
      description: 'Public project-analysis and MCP validation workspace for smoke-tested context tooling.',
    },
  ],
  capabilities: [
    {
  ...
  markdownDocuments: [
    ...
    {
      id: 'doc-mcp-smoke-client',
      title: 'MCP Smoke Client & Container Validation',
      path: 'mcp-server/README.md',
      purpose: 'qa',
      status: 'active',
      summary: 'Documents the local authenticated MCP smoke client, Docker container checks, protocol discovery, read-only tool calls, and audit-log validation.',
      repositoryIds: ['playground'],
      featureIds: ['brief-builder'],
      sensitivity: 'public',
      lastReviewedAt: '2026-08-24',
      tags: ['mcp', 'smoke-test', 'docker', 'security'],
      alignmentTargets: ['MCP platform MVP', 'Security baseline', 'QA smoke checks'],
      auditFindings: ['Confirm every MCP container build validates auth rejection, server discovery, scoped tool listing, tool calls, tenant metadata, and audit-log safety before external agent testing.'],
      appliesTo: ['mcp-server', 'agent-integration', 'container-smoke-test'],
      requiredChecks: [
        'Run npm run mcp:smoke against the container before connecting real agents.',
        'Confirm tools/list returns deterministic read-only annotated tools.',
        'Confirm unauthenticated /mcp responses include authorization guidance.',
      ],
      agentUseLevel: 'required',
      resources: [
        {
          id: 'resource-mcp-discovery',
          title: 'MCP server discovery',
          url: 'https://modelcontextprotocol.io/specification/draft/server/discover',
          type: 'official-docs',
          tags: ['mcp', 'discovery'],
          notes: 'Use this to verify server/discover returns serverInfo, supported versions, capabilities, and instructions.',
        },
        {
          id: 'resource-mcp-tool-annotations',
          title: 'MCP tool annotations risk vocabulary',
          url: 'https://blog.modelcontextprotocol.io/posts/2026-03-16-tool-annotations/',
          type: 'official-docs',
          tags: ['mcp', 'tools', 'security'],
          notes: 'Use readOnlyHint, destructiveHint, idempotentHint, and openWorldHint to make tool risk explicit.',
        },
      ],
    },
  ],
  workItems: [
    ...
    {
      id: 'work-mcp-agent-integration',
      title: 'Connect first real agent to MCP smoke-tested server',
      type: 'integration',
      status: 'todo',
      priority: 'high',
      summary: 'After the local smoke client passes, connect one real MCP-capable agent and confirm it discovers and uses the read-only context tools correctly.',
      nextAction: 'Choose the first agent host and provide a real local client policy with scoped credentials.',
      source: 'MCP smoke client',
      featureIds: ['brief-builder'],
      repositoryIds: ['playground'],
      tags: ['mcp-platform-mvp', 'agent-integration', 'smoke-test'],
      acceptanceCriteria: [
        'The agent can call server/discover and tools/list.',
        'The agent only sees tools allowed by its scopes and license plan.',
        'The agent uses returned context without leaking private project data.',
      ],
  ...
  qaItems: [
    {
      id: 'qa-mcp-container-smoke',
      title: 'MCP container smoke client validation',
      type: 'automated-test',
      status: 'passed',
      priority: 'critical',
      summary: 'Run the authenticated local smoke client against the Dockerized MCP server and confirm health, auth rejection, discovery, tool listing, tool calls, tenant metadata, and audit-log safety.',
      featureIds: ['brief-builder'],
      repositoryIds: ['playground'],
      fileRefs: ['mcp-server/src/smokeClient.ts', 'mcp-server/README.md', 'mcp-server/audit/mcp-audit-smoke.jsonl'],
      automationCoverage: 'automated',
      sourceDoc: 'mcp-server/README.md',
      sourceSection: 'Local Smoke Client',
      nextAction: 'Repeat with a real local client policy before connecting the first external agent.',
      notes: [
        {
          id: 'note-mcp-smoke-1',
          body: 'Smoke client validates MCP best-practice signals before model-driven agent testing.',
          createdAt: '2026-08-24',
        },
      ],
    },
